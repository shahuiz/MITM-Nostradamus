...
def MatchThruARK(m: gp.Model, Nb, xw, yw, kw, meet, dom):
    ...
    m.addConstr(dom == gp.quicksum(meet.flatten())*2)
    return
...
def MatchThruMC(m: gp.Model, Nb, MC_b, MC_r, MC_w, AK_b, AK_r, AK_w, meet, dom):
    ...
    m.addConstr(gp.quicksum(MC_b[j].flatten()) + gp.quicksum(AK_b[j].flatten()) >= 1)
    m.addConstr(gp.quicksum(MC_r[j].flatten()) + gp.quicksum(AK_r[j].flatten()) >= 1)
    return

def solver_AES(KeyLen:int, TOTAL:int, ENCST:int, MATCH:int, KEYST:int, control_panel, dir):
    BlkLen = 128

    # define optimization model
    m = gp.Model('%d-%d-%d-%d-%d-%d' % (BlkLen, KeyLen, TOTAL, ENCST, MATCH, KEYST))
    ...
    SB_match = np.asarray(m.addVars(Nb, NROW, vtype=GRB.BINARY, name='SB_match').values()).reshape((Nb, NROW))
    ...
    for j in range(Nb):
        for i in range(NROW):
            ...
            m.addConstr(Tag_b[j,i] + Tag_r[j,i] <= 1)
    ...
    for r in range(TOTAL):
        ...
        if r == MATCH and MATCH == TOTAL - 1:
            ...
            MatchThruARK(m, Nb, SB_w[r], MC_w[r], Tag_w, SB_match, DOM)

        # match through MixCol
        elif r == MATCH:
            print('mat', r)
        ...
        # last round
            ...
            print('fwd lastr', r)
            AddRoundKey(m, Nb, MC_b[r], MC_r[r], MC_w[r], Tag_b, Tag_r, Tag_w, SB_b[r], SB_r[r], SB_w[r], xor_cost_fwd[nr], xor_cost_bwd[nr])
            SubBytes(m, Nb, SB_b[r], SB_r[r], SB_w[r], SR_b[0], SR_r[0], SR_w[0])
        
        elif r == TOTAL - 1 and r in bwd:
            print('bwd lastr', r)
            SubBytes(m, Nb, SR_b[0], SR_r[0], SR_w[0], SB_b[r], SB_r[r], SB_w[r])
            AddRoundKey(m, Nb, SB_b[r], SB_r[r], SB_w[r], Tag_b, Tag_r, Tag_w, MC_b[r], MC_r[r], MC_w[r], xor_cost_fwd[nr], xor_cost_bwd[nr])
    ...
    m.addConstr(gp.quicksum(MC_r[TOTAL-1].flatten()) == blrdTag) 
    ...
    if control_panel["Quantum"]:
        ...
        m.addConstr(d1 >= d2)
        m.addConstr(dq == d1 - d2)

        m.addConstr(OBJ == gp.min_(dq, d1, d2, dm))
        m.addConstr(OBJ >= SCALE*blrdTag + 1)
        # MITM complexity
        m.addConstr(MITM_cplx == (128 - kw - NBYTE/SCALE*OBJ)/2)
        # Diamond complexity
        m.addConstr(Diamond_cplx == (2*kbr + 128)/3 + 1)
        # Overall complexity
        ...
        m.addConstr(OBJ >= SCALE*blrdTag + 1)
        # MITM complexity
        m.addConstr(MITM_cplx == 128 - kw - NBYTE/SCALE*OBJ)
        # Diamond complexity
        m.addConstr(Diamond_cplx == (kbr + 128)/2)
        # Overall complexity
        m.addConstr(Cplx == gp.max_(MITM_cplx, Diamond_cplx))

    m.addConstr(Cplx >= control_panel["LB"])
    m.addConstr(Cplx <= control_panel["UB"])
